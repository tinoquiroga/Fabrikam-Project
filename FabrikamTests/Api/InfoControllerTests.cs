...
public class InfoControllerTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task GetInfo_ReturnsValidEnvironment()
    {
        ...
        environment.Should().BeOneOf("Development", "Staging", "Production", "dev", "staging", "prod");
    }
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/info/dependencies");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/info/configuration");
    ...
    public async Task GetInfo_IsIdempotent()
    {
        ...
        var response1 = await _client.GetAsync("/api/info");
        var content1 = await response1.Content.ReadAsStringAsync();
        
        var response2 = await _client.GetAsync("/api/info");
