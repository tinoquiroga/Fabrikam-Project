...
[Trait("Category", "Api")]
public class OrdersControllerTests : AuthenticationTestBase
{
    public OrdersControllerTests(AuthenticationTestApplicationFactory factory) 
        : base(factory)
    {
    }
    ...
    {
        // Act
        var response = await AuthenticatedClient.GetAsync("/api/orders/analytics");
    ...
    public async Task GetSalesAnalytics_WithDateRange_ReturnsFilteredData()
    {
        ...
        // Act
        var response = await AuthenticatedClient.GetAsync($"/api/orders/analytics?fromDate={fromDate}&toDate={toDate}");
    ...
    public async Task GetOrder_WithValidId_ReturnsOrder()
    {
        ...
        var ordersResponse = await AuthenticatedClient.GetAsync("/api/orders");
            ...
            ordersResult.RootElement.GetArrayLength() > 0)
        {
            ...
            if (firstOrder.TryGetProperty("id", out var idElement))
            {
                ...
                // Act
                var response = await AuthenticatedClient.GetAsync($"/api/orders/{orderId}");
    ...
    {
        // Act
        var response = await AuthenticatedClient.GetAsync("/api/orders/999999");
