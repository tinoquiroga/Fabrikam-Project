...
# JWT token retrieval utility
function Get-JwtToken {
    param(
        [string]$ApiBaseUrl,
        [string]$Username,  # Actually email for this API
        [string]$Password,
        [int]$TimeoutSec = 30
    )
...
function Get-AuthenticatedToken {
    ...
    try {
        ...
        return Get-JwtToken -ApiBaseUrl $ApiBaseUrl -Username $user.Email -Password $user.Password -TimeoutSec $TimeoutSec
    }
    catch {
        return @{ Success = $false; Error = $_.Exception.Message }
    }
...
function Get-JwtToken {
    ...
    # If called with username/password, use them directly
    if ($Username -and $Password) {
        try {
            ...
            $response = Invoke-RestMethod -Uri "$ApiBaseUrl/api/auth/login" -Method Post -Body $loginRequest -Headers $headers -TimeoutSec $TimeoutSec
        ...
        # Default to Admin role if no credentials provided
